...
def predict_winner(home_team, away_team):
    ...
    # Correctly order the input data with 15 features, including the home_advantage
    input_data = [
        le.transform([home_team])[0],  # home team
        le.transform([away_team])[0],  # away team
        season_averages['home_advantage'].mean(),  # Ensure home advantage is included
    ...
    # Ensure input_data contains all 15 features expected by the scaler
    if len(input_data) != 15:
        raise ValueError(f"Expected 15 features, but got {len(input_data)} features.")
    ...
    return 'Home Team Wins' if prediction >= 0.5 else 'Away Team Wins'

# Example prediction
print(predict_winner("GSW", "LAL"))
